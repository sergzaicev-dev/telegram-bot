...
if 'BOT_TOKEN' in all_env_vars:
    ...
    # 3. Покажем токен полностью (скрывая середину для безопасности)
    if len(token) > 20:
    ...
    if ':' in token:
        ...
        # Проверка ID бота
        if parts[0].isdigit() or (parts[0][0].isdigit() and any(c.isalpha() for c in parts[0])):
            print(f"✅ ID бота выглядит нормально")
        else:
            print(f"⚠️ ID бота странный: {parts[0]}")
    else:
        print(f"❌ КРИТИЧЕСКАЯ ОШИБКА: Нет двоеточия в токене!")
        print(f"   Текущий токен: '{token}'")
    
    # 6. Проверим на пробелы
    if ' ' in token:
        print(f"❌ ПРОБЛЕМА: В токене есть пробелы!")
        print(f"   Токен с пробелами: '{token}'")
        print(f"   Рекомендация: Удалите пробелы на Render")
    else:
        print(f"✅ Пробелов нет")
        
    # 7. Проверим на спецсимволы
    if token.startswith("'") or token.startswith("`") or token.startswith("‘"):
        print(f"❌ ПРОБЛЕМА: Токен начинается с кавычки/апострофа!")
        print(f"   Удалите ' ‘ ` в начале токена на Render")
    if token.endswith("'") or token.endswith("`") or token.endswith("’"):
        print(f"❌ ПРОБЛЕМА: Токен заканчивается кавычкой!")
        print(f"   Удалите ' ` ’ в конце токена на Render")
        
else:
    print(f"❌ BOT_TOKEN не найден в переменных окружения!")
    print(f"   Проверьте настройки на Render: Environment → BOT_TOKEN")
